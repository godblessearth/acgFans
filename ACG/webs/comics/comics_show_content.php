...
function pages($ceil_length){				//封装的函数，用于内容分页，整部作品的上下部切换
	...
	if($id>1&&$id<$max){                     //在最后一项和第一项之间的页面处理
		echo "<a href='comics_show_content.php?id=$id_back'><button class='head_button'>上一部</button></a>";
		echo "<a href='comics_show_content.php?id=$id_next'><button class='head_button'>下一部</button></a>";}
	elseif($id==1){							//最后一项作品页面
			echo "<a href='comics_show_content.php?id=$id_back'><button class='head_button'>上一部</button></a>";
			echo "<a href='comics_show.php'><button class='head_button'>到头了啦</button></a>";}
	elseif($id==$max){						//第一项作品页面
		echo "<a href='comics_show.php'><button class='head_button'>最新的啦</button></a>";
		echo "<a href='comics_show_content.php?id=$id_next'><button class='head_button'>下一部</button></a>";
	}
	echo "<button class='head_button'>第 $num 页</button>";
	echo "<a href='comics_show.php'><button class='head_button'>返回漫画区</button></a>";
	echo "</div>";
	...
	echo "<div class='change_box_up'><a href='comics_show_content.php?ceil_length=5&id=$id'>首页</a></div>";
	if($up<=5){								//判断并设定上一页链接的ceil_length
	//上一页
		echo "<div class='change_box_up'><a href='comics_show_content.php?ceil_length=5&id=$id'>上一页</a></div>";}
	else{
		echo "<div class='change_box_up'><a href='comics_show_content.php?ceil_length=$up&id=$id'>上一页</a></div>";}
	
	//中间页面链接按钮
	//下面判断总分页数，下于6就全部打印页面链接。否则，按当前页面序列打印
	if($total_pages<=6){$j=5;
		for ($i=1;$i<=$total_pages;$i++){
		echo "<div class='change_box'><a href='comics_show_content.php?ceil_length=$j&id=$id' class='change'>$i</a></div>";
	$j+=5;}}
	else{
	if($num_up!=0){
	echo "<div class='change_box'><a href='comics_show_content.php?ceil_length=$up&id=$id' class='change'>$num_up</a></div>";}
	echo "<div class='change_box'><a href='comics_show_content.php?ceil_length=$ceil_length&id=$id' class='change'>$num</a></div>";
		$j=5;
		for ($i=$num_down;$i<=$num_end&&$i<=$total_pages;$i++){
		echo "<div class='change_box'><a href='comics_show_content.php?ceil_length=$down&id=$id' class='change'>$i</a></div>";
	$down+=5;}}
	//下一页
	if($down>=$down_end){					//判断并设定下一页链接的ceil_length
		echo "<div class='change_box_down'><a href='comics_show_content.php?ceil_length=$down_limit&id=$id'>下一页</a></div>";}
	else{
		echo "<div class='change_box_down'><a href='comics_show_content.php?ceil_length=$down&id=$id'>下一页</a></div>";}
		echo "<div class='change_box_down'><a href='comics_show_content.php?ceil_length=$end_pages&id=$id' class='change'>末页</a></div>";
	echo "</div>";
	echo "</div>";
	}
