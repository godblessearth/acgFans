...
function showpages($ceil_length){			//封装的函数，用于作品列表展示分页
	...
	$select_limit="select * from animate order by id desc limit $floor_length,$ceil_length";
	$selectMax="select id from animate where id=(select MAX(id) from animate)";	
	...
	echo "<div class='img_box_content'><a href='../play/play_content.php?title={$row['title']}&url={$row['url']}' target='_blank'><img src='{$row['picture']}'></a><br/></div><div class='img_p_content'><a href='../play/play_content.php?title={$row['title']}&url={$row['url']}' target='_blank'><p>{$row['title']}</p></a></div>";
		echo "</div>";
	}
	echo "</div>";
	...
	echo "<div class='change_box_up'><a href='animates_show.php?ceil_length=9'>首页</a></div>";
	//上一页
	if($up<=9){								//判断并设定上一页链接的ceil_length
		echo "<div class='change_box_up'><a href='animates_show.php?ceil_length=9'>上一页</a></div>";}
	else{
		echo "<div class='change_box_up'><a href='animates_show.php?ceil_length=$up'>上一页</a></div>";}
		
	//中间分页按钮链接区
	if($total_pages<=6){   //如果需要的分页总数不大于6，直接循环echo链接按钮
		$j=9;
		for ($i=1;$i<=$total_pages;$i++){
		echo "<div class='change_box'><a href='animates_show.php?ceil_length=$j' class='change'>$i</a></div>";
		$j=$j+9;}
	}
	else{					//如果需要的分页总数大于6，则固定显示当前页，然后循环打印当前页之后的5个链接按钮。
		if($num_up!=0){		//防止出现第0页。第一页默认6个分页链接。第二页开始每页7个，此时当前页起到锚的作用，作为左右链接的起点
		echo "<div class='change_box'><a href='animates_show.php?ceil_length=$up' class='change'>$num_up</a></div>";}
		echo "<div class='change_box'><a href='animates_show.php?ceil_length=$ceil_length' class='change'>$num</a></div>";
			for ($i=$num_down;$i<=$num_end&&$i<=$total_pages;$i++){
			echo "<div class='change_box'><a href='animates_show.php?ceil_length=$down' class='change'>$i</a></div>";
		$down+=5;}
	}
	
	//下一页
	if($down>=$down_end){					//判断并设定下一页链接的ceil_length
		echo "<div class='change_box_down'><a href='animates_show.php?ceil_length=$down_limit'>下一页</a></div>";}
	else{
		echo "<div class='change_box_down'><a href='animates_show.php?ceil_length=$down'>下一页</a></div>";}
		echo "<div class='change_box_down'><a href='animates_show.php?ceil_length=$end_pages' class='change'>末页</a></div>";
	echo "</div>";
	echo "</div>";
